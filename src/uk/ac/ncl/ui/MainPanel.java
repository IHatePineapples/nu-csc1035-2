...
public class MainPanel extends JFrame {
    ...
    /**
     * Displays the final score then closes the game.
     */
     ...
     *
     * @param moveChecker - main logic of the game
     */
    private void CheckNextMove(MoveChecker moveChecker){
        if (missedMoves>1) endGame();
        else
            moveChecker.colourPieces(grayCells, CellStatus.GRAY);
    }
     ...
     *
     * @param opponent - opponent's selected piece
     */
    ...
    private void MovePlayer(Cell tempEl){
        if (tempEl.getMove()!=null) {
            ...
            if (opponent!=null) {
                ...
                grayCells = moveChecker.findPotentialMoves(PLAYERS_CELL_STATUS);
                moveChecker.colourPieces(grayCells, CellStatus.GRAY);
                if ((moveChecker.findPotentialMoves(PLAYERS_CELL_STATUS).isEmpty() && moveChecker.findPotentialMoves(OPPONENTS_CELL_STATUS).isEmpty() )) {
                    endGame();
                } else {
                    Cell opponent2 = moveChecker.generateOpponent(OPPONENTS_CELL_STATUS);
                    if (moveChecker.findPotentialMoves(PLAYERS_CELL_STATUS).isEmpty() && opponent2!=null) {
                        opponent.colourTemp(OPPONENTS_COLOUR, true);
     ...
     *
     * @param tempEl - selected piece by click
     */
     ...
     *
     * @param taskPerformer - action event
     */
